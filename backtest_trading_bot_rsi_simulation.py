...
class BacktestConfig:
    ...
    trigger_on_intrabar_RSI: bool = True  # New flag to enable intrabar RSI simulation
...
def parse_args() -> BacktestConfig:
    ...
    parser.add_argument('--no_intrabar_rsi', action='store_true',
                      help='Disable intrabar RSI simulation (use candle-close only)')
    ...
    return BacktestConfig(
        ...
        trigger_on_intrabar_RSI=not args.no_intrabar_rsi
    )
...
def backtest(df: pd.DataFrame, ta: TechnicalAnalysis, config: BacktestConfig) -> BacktestResults:
    ...
    for i in range(50, len(df)):  # Start after EMA period
        ...
        # Use intrabar RSI simulation if enabled
        if config.trigger_on_intrabar_RSI:
            signal, entry, rsi_value, relative_volume, trend_slope = simulate_intrabar_rsi(
                df, ta, config.product_id, i, RSI_THRESHOLD
            )
        else:
            # Use standard candle-close analysis
            signal, entry, rsi_value, relative_volume, trend_slope = analyze(historical_df, ta, config.product_id)
            # Log the signal for comparison with live logs if a signal is generated
            if signal:
                log_signal(df.index[i], rsi_value, "Candle-close RSI", config.product_id, entry)
        ...
        # Enter new trade if signal and no position
            ...
            current_price = entry if config.trigger_on_intrabar_RSI else df.iloc[i]['close']
            atr = ta.compute_atr(historical_df.to_dict('records'))
...
def log_signal(timestamp: datetime, rsi_value: float, reason: str, product_id: str, price: float):
    """Log a signal for comparison with live logs."""
    logger.info(f"[SIGNAL LOG] {timestamp.strftime('%Y-%m-%d %H:%M:%S')} | "
                f"Product: {product_id} | RSI: {rsi_value:.2f} | "
                f"Price: {price:.2f} | Reason: {reason}")

def simulate_intrabar_rsi(df: pd.DataFrame, ta: TechnicalAnalysis, product_id: str, 
                         current_idx: int, rsi_threshold: float = RSI_THRESHOLD) -> Tuple[bool, float, float, float, float]:
    """
    Simulate intrabar RSI values to detect when RSI crosses below threshold during a candle.
    ...
    Returns:
        Tuple of (signal, entry_price, rsi_value, relative_volume, trend_slope)
    """
    # Get the current candle
    current_candle = df.iloc[current_idx]
    previous_candle = df.iloc[current_idx-1]
    
    # Get historical data up to the previous candle
    historical_df = df.iloc[:current_idx]
    historical_candles = historical_df.to_dict('records')
    
    # Calculate RSI for the previous candle
    rsi_previous = ta.compute_rsi(product_id, historical_candles, period=14)
    
    # Calculate average volume for relative volume
    avg_volume = df["volume"].iloc[max(0, current_idx-VOLUME_LOOKBACK):current_idx].mean()
    relative_volume = current_candle['volume'] / avg_volume if avg_volume > 0 else 0
    
    # Calculate trend slope
    trend_slope = 0.0
    try:
        if len(historical_df) >= 5:
            close_prices = historical_df['close'].tail(5).values
    ...
    # Check if RSI was below threshold on previous candle
    rsi_triggered = rsi_previous < rsi_threshold
    
    if rsi_triggered:
        # Simulate intrabar price movement to estimate when RSI would cross below threshold
        # We'll use the midpoint of the candle as a simple approximation
        midpoint_price = (current_candle['high'] + current_candle['low']) / 2
        
        # Create a simulated candle with the midpoint price
        simulated_candle = {
        ...
        # Add the simulated candle to historical data
        simulated_candles = historical_candles + [simulated_candle]
        
        # Calculate RSI with the simulated candle
        rsi_simulated = ta.compute_rsi(product_id, simulated_candles, period=14)
        
        # Log the simulated RSI value
        logger.info(f"[INTRABAR SIMULATION] Candle: {current_candle.name}, "
                   f"Previous RSI: {rsi_previous:.2f}, Simulated RSI: {rsi_simulated:.2f}")
        
        # If simulated RSI is below threshold, generate a signal
        if rsi_simulated < rsi_threshold:
            logger.info(f"[INTRABAR SIGNAL] RSI crossed below {rsi_threshold} during candle at {current_candle.name}")
            # Log the signal for comparison with live logs
            log_signal(current_candle.name, rsi_simulated, "Intrabar RSI simulation", product_id, midpoint_price)
            return True, midpoint_price, rsi_simulated, relative_volume, trend_slope
...
def main():
    ...
    try:
        ...
        # Log intrabar RSI simulation mode
        if config.trigger_on_intrabar_RSI:
            logger.info("Intrabar RSI simulation ENABLED - will detect RSI crossing below threshold during candles")
        else:
            logger.info("Intrabar RSI simulation DISABLED - will only detect RSI signals at candle close")
