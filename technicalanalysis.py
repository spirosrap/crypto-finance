    ...
    def compute_macd(self, product_id: str, candles: List[Dict]) -> Tuple[float, float, float]:
        ...
        try:
            ...
            if len(prices) < min_periods:
                self.logger.warning(f"Insufficient data for MACD calculation. Need {min_periods} periods, got {len(prices)}")
                return 0.0, 0.0, 0.0  # Return neutral values instead of raising an error
            ...
            # Get the last valid values, handling NaN values
            macd_value = float(macd_line[-1]) if not np.isnan(macd_line[-1]) else 0.0
            signal_value = float(signal_line[-1]) if not np.isnan(signal_line[-1]) else 0.0
            ...
            if price_scale != 0:  # Avoid division by zero
            ...
            return 0.0, 0.0, 0.0  # Return neutral values on error
