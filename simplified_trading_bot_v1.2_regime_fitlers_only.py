...
def analyze(df: pd.DataFrame, ta: TechnicalAnalysis, product_id: str):
    ...
    regime = classify_market_regime(atr, current["close"])
    
    # Log regime information
    logger.info(f"Current market regime: {regime}")
    
    # Check if we should skip trading based on regime
    if regime == "sideways":
        logger.info(f"[SKIPPED] Trade signal detected but skipped due to sideways regime (ATR: {atr:.4f})")
        return False, None
    
    # Original entry logic remains unchanged
    if (
        rsi < RSI_THRESHOLD
