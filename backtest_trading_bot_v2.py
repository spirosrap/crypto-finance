...
class Trade:
    ...
    setup_type: str = "RSI Dip"  # Type of setup (RSI Dip or Breakout)
    breakout_pct: float = 0.0  # Percentage of breakout (for Breakout setup)
    side: str = "LONG"  # Trading direction (LONG or SHORT)
...
def export_trades_to_csv(trades: List[Trade], product_id: str) -> str:
    ...
    try:
        ...
        trades_df['Leverage'] = '5x'  # Fixed leverage for backtest
        ...
        trades_df['Setup Type'] = trades_df['setup_type']
        ...
        # Add the new columns
        trades_df['Breakout Range'] = trades_df['breakout_pct'].round(2)
        trades_df['Entry Type'] = trades_df['side']
...
def backtest(df: pd.DataFrame, ta: TechnicalAnalysis, config: BacktestConfig) -> BacktestResults:
    ...
    for i in range(50, len(df)):  # Start after EMA period
        ...
        signal, entry, rsi_value, relative_volume, trend_slope, side, setup_type, breakout_pct = analyze(historical_df, ta, config.product_id)
        ...
        if current_trade:
            ...
            if current_high >= tp_price:
                ...
                trades.append(Trade(
                    ...
                    breakout_pct=current_trade['breakout_pct'],
                ...
                # Update balance history and track potential drawdown
                balance_history.append(balance)
                peak_balance = max(peak_balance, balance)

            # Check if low price reached or went below stop loss
            elif current_low <= current_trade['entry_price'] * (1 - SL_PERCENT):
                loss = (current_trade['entry_price'] * (1 - SL_PERCENT) - current_trade['entry_price']) * current_trade['size'] * config.leverage
                balance += loss
                # Determine if SL was hit by wick or close
                exit_reason = "SL WICK HIT" if current_price > current_trade['entry_price'] * (1 - SL_PERCENT) else "SL HIT"
                trades.append(Trade(
                    entry_time=current_trade['entry_time'],
                    exit_time=df.index[i],
                    entry_price=current_trade['entry_price'],
                    exit_price=current_trade['entry_price'] * (1 - SL_PERCENT),
                    profit=loss,
                    type='SL',
                    atr=atr,
                    atr_percent=(atr / current_trade['entry_price']) * 100,
...
def print_results(results: BacktestResults):
    ...
    # Calculate setup type statistics
    rsi_trades = [t for t in results.trades if t.setup_type == "RSI Dip"]
    breakout_trades = [t for t in results.trades if t.setup_type == "Breakout"]
    
    rsi_wins = sum(1 for t in rsi_trades if t.profit > 0)
    breakout_wins = sum(1 for t in breakout_trades if t.profit > 0)
    
    rsi_win_rate = (rsi_wins / len(rsi_trades) * 100) if rsi_trades else 0
    breakout_win_rate = (breakout_wins / len(breakout_trades) * 100) if breakout_trades else 0
    
    setup_data = [
        ["RSI Dip Trades", len(rsi_trades)],
        ["RSI Dip Win Rate", f"{rsi_win_rate:.2f}%"],
        ["Breakout Trades", len(breakout_trades)],
        ["Breakout Win Rate", f"{breakout_win_rate:.2f}%"]
    ]
    ...
    print(tabulate(setup_data, tablefmt="grid"))
