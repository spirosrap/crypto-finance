...
def analyze(df: pd.DataFrame, ta: TechnicalAnalysis, product_id: str):
    ...
    logger.info(f"Analysis: RSI={rsi:.2f}, Volume={current['volume']:.2f} > Avg={avg_volume:.2f}, EMA Slope={ema_slope:.4f} > Threshold={EMA_SLOPE_THRESHOLD:.4f}")

    if (
        rsi < RSI_THRESHOLD
        and current["volume"] > avg_volume
        and ema_slope > EMA_SLOPE_THRESHOLD  # New trend filter condition
    ):
