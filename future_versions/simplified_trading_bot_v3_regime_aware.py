...
def analyze(df: pd.DataFrame, ta: TechnicalAnalysis, product_id: str):
    ...
    # Entry filter by regime
    # Allow RSI-based reversal entries only when:
    # - Volatility is Weak or Moderate (ATR% ≤ 1.0)
    # - Trend is Sideways (EMA slope < 0.1%)
    regime_allows_entry = (atr_percent <= 1.0) and (abs(ema_slope) < 0.001)
    
    if (
        rsi < RSI_THRESHOLD
        and current["volume"] > avg_volume
        and ema_slope > EMA_SLOPE_THRESHOLD  # Original trend filter condition
        and regime_allows_entry  # New regime filter condition
    ):
        logger.info(f"[SIGNAL] BUY {product_id} at {current['close']:.2f} in {market_regime} regime")
        return True, current["close"], market_regime
    
    if rsi < RSI_THRESHOLD and current["volume"] > avg_volume and ema_slope > EMA_SLOPE_THRESHOLD and not regime_allows_entry:
        logger.info(f"[BLOCKED] RSI signal blocked due to unfavorable regime: {market_regime}")
...
def execute_trade(cb, entry_price: float, product_id: str, margin: float, leverage: int, market_regime: str = "Unknown/Unknown"):
...
def main():
    ...
    try:
        ...
        # Live trading mode
        signal, entry, market_regime = analyze(df, ta, args.product_id)
        
        if signal:
            logger.info(f"[SIGNAL] BUY {args.product_id} at {entry:.2f}")
            if execute_trade(cb, entry, args.product_id, args.margin, args.leverage, market_regime):
