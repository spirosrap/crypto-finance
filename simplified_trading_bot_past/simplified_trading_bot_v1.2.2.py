# Version: v1.2.1f
# Logic: RSI Dip + refined pre-entry filters (UNCERTAIN regime protection, bear trap filter)
# Extras: Confidence scoring active (not used for filtering)
# Phase: Data accumulation + score evaluation
...
def analyze(df: pd.DataFrame, ta: TechnicalAnalysis, product_id: str):
    ...
    # Apply confidence score deductions
    if market_regime == "UNCERTAIN":
        confidence_score -= 0.3
    if relative_volume < 1.5:
        confidence_score -= 0.2
    if atr_percent < 0.2 and abs(trend_slope) < 0.001:
        confidence_score -= 0.2
    if rsi_current > 27:
        confidence_score -= 0.1
    if market_regime == "TRENDING" and trend_slope < -0.001 and rsi_current < 30 and relative_volume < 0.6:
        confidence_score -= 0.3
        
    # Ensure confidence score stays within bounds
    confidence_score = max(0.0, min(1.0, confidence_score))
    
    # Log confidence score and context
    logger.info(f"[CONFIDENCE] Score: {confidence_score:.2f} | Regime={market_regime}, RSI={rsi_current:.1f}, Vol={relative_volume:.1f}, Slope={trend_slope:.4f}, ATR%={atr_percent:.2f}")
    ...
    # Filter 4: Reject trades in strongly bearish conditions
    if market_regime == "TRENDING" and trend_slope < -0.002:
        logger.info(f"[FILTERED] Rejected trade: Strong bearish trend detected (Trend Slope={trend_slope:.4f})")
        return False, None, None, None, None, None
    ...
    if rsi_triggered and rsi_confirmed:
        ...
        return True, current["close"], rsi_current, relative_volume, trend_slope, confidence_score    
...
def execute_trade(cb, entry_price: float, product_id: str, margin: float, leverage: int, trend_slope: float = None, confidence_score: float = None):
    ...
    try:
        ...
        try:
            ...
            new_trade = pd.DataFrame([{
                ...
                'Confidence Score': confidence_score  # Added confidence score
            }])
...
def main():
    ...
    try:
        ...
        # Live trading mode
        signal, entry, rsi_value, relative_volume, trend_slope, confidence_score = analyze(df, ta, args.product_id)
        
        if signal:
            logger.info(f"[SIGNAL] BUY {args.product_id} at {entry:.2f}")
            if execute_trade(cb, entry, args.product_id, args.margin, args.leverage, trend_slope, confidence_score):
