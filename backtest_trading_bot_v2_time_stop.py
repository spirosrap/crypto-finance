from simplified_trading_bot import (
    ...
    get_price_precision, analyze, determine_tp_mode, TIME_STOP_BARS, TIME_STOP_THRESHOLD
)
...
def backtest(df: pd.DataFrame, ta: TechnicalAnalysis, config: BacktestConfig) -> BacktestResults:
    ...
    for i in range(50, len(df)):  # Start after EMA period
        ...
        if current_trade:
            ...
            # TIME STOP LOGIC (added)
            # Check if we are at the TIME_STOP_BARS after entry
            time_stop_applied = False
            trade_bars_elapsed = i - current_trade['entry_bar']
            
            if trade_bars_elapsed == TIME_STOP_BARS:
                # Calculate highest price reached during the 3 bars
                entry_idx = current_trade['entry_bar']
                time_stop_bars = df.iloc[entry_idx+1:i+1]  # Bars after entry up to current
                
                # Make sure we have exactly TIME_STOP_BARS
                if len(time_stop_bars) == TIME_STOP_BARS:
                    highest_price = time_stop_bars['high'].max()
            ...
            # If time stop wasn't applied, check TP and SL
            if not time_stop_applied:
        ...
        # Enter new trade if signal and no position
            ...
            current_trade = {
                ...
                'mae': 0.0,
                'mfe': 0.0,
                'entry_bar': i  # Track entry bar index for time stop
            }
            position = size
    ...
    # Log time stop statistics
    logger.info(f"Total time stop exits: {time_stop_exits} of {len(trades)} trades ({time_stop_exits/len(trades)*100:.2f}%)")
...
def print_results(results: BacktestResults):
    ...
    # Time stop statistics
    time_stop_trades = [t for t in results.trades if t.exit_reason == "TIME STOP"]
    time_stop_wins = sum(1 for t in time_stop_trades if t.profit > 0)
    time_stop_win_rate = (time_stop_wins / len(time_stop_trades) * 100) if time_stop_trades else 0
    
    exit_reason_data = [
        ["Time Stop Exits", len(time_stop_trades)],
        ["Time Stop Win Rate", f"{time_stop_win_rate:.2f}%"]
    ]
    ...
    print(tabulate(exit_reason_data, tablefmt="grid"))
