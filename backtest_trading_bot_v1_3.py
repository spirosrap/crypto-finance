from simplified_trading_bot_v1_3_improved_entry import (
...
def fetch_candles(cb: CoinbaseService, product_id: str, 
    ...
    # Add body_size and moving averages
...
def backtest(df: pd.DataFrame, ta: TechnicalAnalysis, config: BacktestConfig) -> BacktestResults:
    ...
    for i in range(50, len(df)):  # Start after EMA period
        ...
        signal, entry_price, position_size_multiplier = analyze(historical_df, ta, config.product_id)
        ...
        elif signal and entry_price:
            # Calculate position size with multiplier
            position_size = balance * 0.1 * position_size_multiplier  # Use 10% of balance per trade, adjusted by multiplier
            
            # Calculate ATR for TP/SL
            atr = ta.compute_atr(historical_df.to_dict('records'))
            tp_mode, tp_price = determine_tp_mode(entry_price, atr)
