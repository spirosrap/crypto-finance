...
# Simplified Trading Bot v1.2.1 (Paper Trading Version)
...
def load_paper_trading_state():
    ...
    return {'positions': [], 'balance': 1000.0}  # Default state
...
def analyze(df: pd.DataFrame, ta: TechnicalAnalysis, product_id: str):
    ...
    # Filter 4: Reject RSI Dip entries in strong bearish trends with weak volume
    if market_regime == "TRENDING" and trend_slope < -0.001 and rsi_current < 30 and relative_volume < 0.6:
        logger.info(f"[FILTERED] Rejected RSI Dip in strong bearish trend with weak volume (Slope={trend_slope:.4f}, RSI={rsi_current:.2f}, Rel Vol={relative_volume:.2f})")
        return False, None, None, None, None        
...
def execute_paper_trade(cb, entry_price: float, product_id: str, margin: float, leverage: int, trend_slope: float = None):
    ...
    try:
        ...
        try:
            ...
            logger.info("Paper trade logged to automated_trades_paper.csv")
            
        except Exception as e:
            logger.error(f"Error logging paper trade to CSV: {e}")
            ...
            if execute_paper_trade(cb, entry, args.product_id, args.margin, args.leverage, trend_slope):
                ...
                try:
                    ...
                    if not trades_df.empty:
                        ...
                        trades_df.to_csv('automated_trades_paper.csv', index=False)
