...
## February 2026
**State at a glance (February recap):** Volatility-heavy start; recent 4h windows show few trades and more stops than wins. Trade flow is constrained by long-heavy suggestions and repeated BTC setups; BTC was excluded from gate-scan entries earlier in the month (kept as the trend/range-break anchor), but was re-enabled on Feb 7. Regime tilt is now active for gate-scan (BTC EMA20, 2-day confirm, 70/30) with imbalance suppression. Entries remain sparse after the regime tilt tweak; currently only BCH is qualifying, and BTC likely would have qualified if it were not excluded. Feb 5 produced no overnight trades on imbalance and only one BCH trade (TP1 hit, TP2 not). Feb 6 saw a sharp BTC selloff down toward ~60K and again no trades due to imbalance; in outsized moves like this, the system typically stands down to avoid getting hurt by reversal whipsaws or by trying to "hope" the reversal. Feb 7–14 continued with elevated ATR and one-sided short pressure, so the system mostly stood down. Recent gate-scan logs suggest ATR is trending down (daily top-15 median ATR has cooled from ~1315 on Feb 6 to ~1009 on Feb 12 in `logs/gate_scan_paper.log`), but conditions remained above the preferred regime and side imbalance stayed severe in bearish tape (for example Feb 13 gate-scan: LONG=2, SHORT=52, output suppressed by reversal protection). Feb 14 remained a no-trade wait day, with BTC ATR closer (`~577 bps`) but still above the current baseline BTC threshold (`<=487.5 bps`, i.e., 1.5x of 325 bps cap). Feb 15 became the first trade day after the pause, Feb 16 added another trade (LTC), and Feb 17 early morning produced one LONG and one SHORT. Feb 18 review shows those two opposite-side trades left performance near breakeven (SEI hit TP1 while LTC is losing), which supports the `docs/plans/tp1_time_stop_plan.md` direction: shorter expiration/time-stop treatment for losing trades. On Feb 19, flow remains active but still not at full speed, with only two live trades (`BTC`/`DOT`) and continued low concurrency versus the usual 6-7 positions. Feb 18 also included fixes for post-upgrade close handling issues tied to the Feb 3 CCXT upgrade path (expired-close/watchdog behavior and REST fallback reporting). Later on Feb 19, a false daily-stop was triggered by a stale BTC partial reconstructed from truncated fill history; watchdog now skips closures when inferred open time predates the earliest visible fill for that product, and the bad closure row was removed from the log. Even with the strategy still tilted toward shorts, the flow has leaned longer than expected; today 4 of 5 trades were LONG despite regime tilt. Entries remain few and still below full speed (usually 6-7 simultaneous positions). Current normalization estimates for the ~300–400 ATR bps zone still point to late February to mid-March in base/fast scenarios, with a slower path extending into early April. Dashboard now includes a Coinbase regime/key-level snapshot (public) plus optional private portfolio sync (balances + recent fills) and position alignment tagging to keep regime context + exposure in one place. Conditions read as bearish for BTC; monitoring PF/expectancy as the system approaches the 150-trade checkpoint. As of Feb 20, the long-side concern was reviewed against logs and appears to be normal flow behavior rather than a regime-tilt failure. As of Feb 21, trade flow is picking up toward a more normal phase; recent outcomes are softer, so evaluation is deferred to the end of the next 50-trade block. Feb 22 saw 5 trades at a time yesterday, then dropped to 2 at logging time (`ETH`/`BTC` open); mixed outcomes remain with a small net loss currently, and regime tilt still appears weak in effect. On Feb 23, yesterday’s trades are now positive and PF is recovering to 1.45 after briefly dipping below 1; there are currently no open trades after reversal-detection conditions are active.
**Summary update (Feb 23 checkpoint policy):** The active mini-checkpoint is now 50 live closes from `2026-02-03` (regime-tilt start), with decisions based on the live dashboard full-closed trade view (partials excluded): `PF >= 1.30` can justify scale-up, `PF 1.00-1.29` stays same size, and `PF < 1.00` triggers descale first then one defensive change (prefer `docs/plans/tp1_time_stop_plan.md`).

## February 23, 2026 - Post-Feb-3 50-Trade Decision Rules
- Anchor this checkpoint to live closes from `2026-02-03` onward (regime-tilt phase), not the older Jan 15 baseline series.
- Use one metric source for consistency: `watchdog_dashboard.py` live mode, start date `2026-02-03`, with partial-only rows excluded.
- Current progress in that view: `31 / 50` trades, PF about `1.04`.
- Decision at 50 trades:
- If `PF >= 1.30` and expectancy is positive and max drawdown is below `10%`: allow a controlled scale-up (`+10-20%`) and keep rules unchanged for the next block.
- If `PF` is in `1.00-1.29`: keep the same size and do not add `tp1_time_stop` yet.
- If `PF < 1.00` (or expectancy is non-positive): descale first (`-25%`), then apply one defensive change only; first choice is `docs/plans/tp1_time_stop_plan.md`.
- If drawdown breaches `15%` at checkpoint, prioritize protection over growth: descale harder (`-50%`) or pause live until metrics stabilize.
