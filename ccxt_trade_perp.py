...
def fetch_reference_price(exchange: ccxt.Exchange, ccxt_symbol: str) -> float:
    ...
    if not last:
        raise RuntimeError("Unable to fetch reference price from ticker.")
    return float(last)
...
def fetch_perp_available_usdc(exchange: ccxt.Exchange) -> float:
    ...
    try:
        ...
        return float(balance.get("free", {}).get("USDC") or 0.0)
    except Exception as exc:
...
def place_entry_order(
    ...
    if dry_run:
        logger.info("[Dry run] create_order(%s, %s, %s, amount=%.8f, price=%s, params=%s)",
                    meta.ccxt_symbol, "limit" if price else "market", side.lower(), amount, price, params)
