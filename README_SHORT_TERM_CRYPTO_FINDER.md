...
- **Short-Line Summaries**: Each candidate ends with a concise one-liner for
  quick triage.
- **Gate Proximity Scan**: Use `scripts/symbol_snapshot.py --gate-scan --profile focused_no_llm_100 --top 15` to see which symbols are closest to clearing the RR/ATR gates (tiered ATR caps applied). Helpful to gauge when conditions are near tradable without loosening filters. Profiles are filter presets (not fixed symbol lists); gate-scan will analyze the profile-filtered universe unless you cap breadth with `--scan-limit N` (for example `--scan-limit 100`). Add `--baseline-commands` for live ccxt command lines and `--baseline-paper-command` to emit a one-shot `baseline_finder_from_snapshot.py` command for paper trades (open live/paper positions are skipped unless `--baseline-include-open` is set). Live baseline commands skip perps missing from `derived_perps_intx.txt` (thousand-unit aliases handled); if the list is missing, validation is skipped.
...
- **RR Drivers Readout**: `scripts/symbol_snapshot.py` now prints a short `RR drivers:` line showing what’s capping reward (e.g., `tp_clamp` vs `tp_rr`) and which risk floor dominates (`raw`, `atr_floor`, `fee`, or `tick`).
- **Rich Tables (ASCII)**: When `rich` is installed, `scripts/symbol_snapshot.py` and `--gate-scan` render compact ASCII tables; output falls back to plain text if `rich` is missing.
...
- **Freqtrade bridge** – export finder results to `signals/freqtrade_signals.json` via
  ...
  `FinderBridgeStrategy` drive a Freqtrade bot (see `docs/pipelines/freqtrade_bridge.md`).
