...
- **Risk Guards (automated)**: daily loss gate (default −2%/−$20 of $1k) and BTC range‑break circuit (7‑day range ±0.5× ATR) suppress new entries until the daily reset or a confirmed daily close re-enters the range. `range_break_confirmed_only` makes the trigger rely on confirmed closes only (less intraday flip‑flop).
