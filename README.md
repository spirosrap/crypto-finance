...
- **Finders**: `short_term_crypto_finder.py`, `long_term_crypto_finder.py`.
  ...
  - Gate proximity view (short-term): `python scripts/symbol_snapshot.py --gate-scan [--scan-limit N] [--balanced]`.
    ...
    - Partial TP (baseline commands): `--baseline-partial-tp-rr` + `--baseline-partial-tp-pct` adds TP1 to live and paper; `--baseline-partial-tp-move-sl` moves SL to entry **paper‑only**. Live TP1 uses two brackets; REST fallback is disabled when TP1 is set.
