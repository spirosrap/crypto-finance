...
class MarketAnalyzerUI:
    ...
    def create_gui(self):
        ...
        # Add time stop monitor toggle button (after simplified trading button)
        self.time_stop_monitoring = False
        self.time_stop_monitor_thread = None
    ...
    def _force_exit(self):
        ...
        try:
            # Stop time stop monitoring if active
            if self.time_stop_monitoring:
                self.toggle_time_stop_monitor()
    ...
    def toggle_time_stop_monitor(self):
        """Toggle the time stop monitor on/off"""
        if not self.time_stop_monitoring:
            # Start time stop monitoring
            self.time_stop_monitoring = True
            self.time_stop_btn.configure(
                text="Stop Time Stop Monitor",
                fg_color="#B22222"  # Fire Brick Red
            )
            self.root.title("Crypto Market Analyzer [Time Stop Monitor ON]")
            self.queue.put(("append", "\nTime stop monitoring started. Checking for 3-bar time stops...\n"))
            self.queue.put(("append", "Logic: Exit if price doesn't reach +0.5% within 3 bars\n"))
            
            # Start the time stop monitoring process using the thread pool
            self.submit_task("time_stop_monitor", self._time_stop_monitor_loop)
        else:
            # Stop time stop monitoring immediately
            self.time_stop_monitoring = False
            ...
            # Cancel the time stop monitoring task without waiting
            with self.thread_lock:
                if "time_stop_monitor" in self.active_futures:
                    future = self.active_futures["time_stop_monitor"]
                    if not future.done():
                        future.cancel()
                    del self.active_futures["time_stop_monitor"]
            
            # Update UI immediately
            self.time_stop_btn.configure(
                text="Start Time Stop Monitor",
                fg_color="#4682B4"  # Back to Steel Blue
            )
            self.root.title("Crypto Market Analyzer")
            self.queue.put(("append", "\nTime stop monitoring stopped.\n"))
            self.queue.put(("status", "Time stop monitoring stopped"))

    def _time_stop_monitor_loop(self):
        ...
        while self.time_stop_monitoring:
            try:
                ...
                cmd = [
                    "python",
                    "time_stop_monitor.py",
                    "--check-interval",
                    "300"  # 5 minute check interval when run via UI
                ]
                ...
                for line in iter(process.stdout.readline, ''):
                    ...
                    # Check if a time stop was triggered
                    if "time stop triggered" in line.lower() or "successfully closed position" in line.lower():
                        self.queue.put(("status", "Time stop triggered! Position closed."))
                        # Force a check of orders/positions to refresh the UI
                        self.check_open_orders_and_positions_ui()
                
                process.wait()
                
                # Sleep between checks (every 5 minutes)
                self.queue.put(("append", "\nWaiting 5 minutes before next time stop check...\n"))
                time.sleep(300)
                
            except Exception as e:
                self.queue.put(("append", f"\nError in time stop monitor: {str(e)}\n"))
